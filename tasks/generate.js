...
module.exports = function (grunt) {
    grunt.registerMultiTask('generate', 'Build generated CSS files.', function () {
            ...
            this.files.forEach(function (filePair) {
                filePair.src.forEach(function (file) {
                    let rv = grunt.file.read(file)
                         .split('\n')
                         .map(line => line.match(/<%= name %>|<%= value %>/g) ? ['*/\n', ...variables.map(obj => line.replace(/<%= value %>/g, obj.value).replace(/<%= name %>/g, obj.name)), '\n/*'].join('\n') : line)
